...
for t=1:F
    ...
    GTsNotMapped=find(~M(t,:) & gtInd(t,:));%记录真实 的轨迹有多少（下标表示）还没有匹配，在当前帧
    EsNotMapped=setdiff(find(stInd(t,:)),M(t,:));%记录我们假定的轨迹还有多少没有匹配，在当前帧
...
%%逐帧迭代
% % 0726把假定轨迹和真实轨迹求距离，将最小的距离的假定放在对应帧的真实轨迹上
            if mindist <= td
    ...
    falsepositives=setdiff(alltrackers,mappedtrackers);%%没有匹配的计入假正FP 
...
%     allfalsepos(t,1:length(falsepositives))=falsepositives;
    ...
    if t>1
        for ct=curtracked
            lastnotempty=find(M(1:t-1,ct),1,'last');%返回最后一个非0的下标
            if gtInd(t-1,ct) && ~isempty(lastnotempty) && M(t,ct)~=M(lastnotempty,ct)
                mme(t)=mme(t)+1;%如果当前的轨迹下标和上一个不匹配则加1
            end
        end
    end
    ...
    for ct=curtracked
        ...
        if options.eval3d
            d(t,ct)=norm([gtInfo.Xgp(t,ct) gtInfo.Ygp(t,ct)] - ...
                [stateInfo.Xgp(t,eid) stateInfo.Ygp(t,eid)]);%真实数据和轨迹数据空间的距离
        else
    ...
    fp(t)=numel(find(stInd(t,:)))-c(t);%假正个数
    m(t)=g(t)-c(t);%遗失个数
...
missed=sum(m);%遗失个数
falsepositives=sum(fp);%假正个数
idswitches=sum(mme);%ids个数

if options.eval3d
    MOTP=(1-sum(sum(d))/sum(c)/td) * 100; % avg distance to [0,100]，d为相互匹配(检测和轨迹)的距离，c为每帧对应的检测个数
else
...
recall=sum(c)/sum(g)*100;%召回个数，被加入估计的检测个数/地面真实检测个数
precision=sum(c)/(sum(fp)+sum(c))*100;%精确度，被加入估计的检测个数/（被加入估计的检测个数+假正个数）
FAR=sum(fp)/Fgt;%平均每帧的假正数
 

%% MT PT ML%%直接计算被赋予标签的轨迹个数，占地面真实的帧数的百分比
...
for i=1:Ngt
    ...
    b(~~b)=1;
    fr(i)=numel(find(diff(b)==-1));%多少次由0变1,就是中间断了，由被接上来
end
FRA=sum(fr);

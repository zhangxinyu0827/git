...
for mot=16
    for training=[1 0]
        %每次重新初始化
        get_det_img_mot_16_det_unnms_init_seq;
...
%               seqData.inputDetections =  fullfile(rootDir,seqDirs{seqNum},'gt',[ 'gt.txt']);    
             seqData.inputDetections = fullfile ('iter_cluster', [seqDirs{seqNum}, '.txt'] )
                end
...
%                     save_img_f333=[img_file333 '/' sprintf('%08d.jpg',tmp_image_i)];

...
% 		/* convert from image to distorted sensor coordinates转化图像到扭曲的传感器坐标 */
Xd = mGeo.mDpx * (Xi - mInt.mCx) / mInt.mSx;
Yd = mGeo.mDpy * (Yi - mInt.mCy);

% 		/* convert from distorted sensor to undistorted sensor plane coordinates 转化扭曲坐标到不失真 传感器地面坐标*/
[Xu Yu]=distortedToUndistortedSensorCoord (Xd, Yd, mInt.mKappa1);

% 		/* calculate the corresponding xw and yw world coordinates	 计算相应的世界坐标*/
% 		/* (these equations were derived by simply inverting等式通过简单反向得到	 */
% 		/* the perspective projection 透视投影equations using Macsyma)	 */
Zw=0;

...
%                if cur_mask(obsPrev(10),cur_observation.g_id(obsInd(i)))==1
% %                    tsp_sc=0;
%                    tsp_sc=exp(cur_tsp_cost(obsPrev(10),cur_observation.g_id(obsInd(i)))/((cur_tsp_gap(obsPrev(10),cur_observation.g_id(obsInd(i)))+1)*other_param.base_tsp_cost))-1;
%                else
%                    tsp_sc=exp(1)-1;
%                end
               ...
               % track score 
%                tmp_rat=1-1./(1+exp(-(  cur_observation.sc(obsInd(i))/100-1/2)*10));
%              tmp_rat=-0.4* cur_observation.sc(obsInd(i))/100+1;
%                 tmp_rat=1-cur_observation.sc(obsInd(i));
%                 loglik = loglikPrev +exp(-cur_observation.sc(obsInd(i)))+ (tmp_rat)*(other_param.appW*log(likelihood_app/other_param.appNullprob) + other_param.motW*LL);                   
                loglik = loglikPrev + (other_param.appW*log(likelihood_app/other_param.appNullprob) + other_param.motW*LL);                   
        
%                 loglik =0.5*tmp_rat+ 0.25*tsp_sc+loglikPrev + (tmp_rat)*(other_param.appW*log(likelihood_app/other_param.appNullprob) + other_param.motW*LL);                   
           end
           confsc = confscPrev + cur_observation.sc(obsInd(i));       
           
           [obsTreeSetPrev branchInd] = obsTreeSetPrev.addnode(treeInd, [cur_observation.x(obsInd(i)) cur_observation.y(obsInd(i)) cur_observation.w(obsInd(i)) cur_observation.h(obsInd(i)) cur_observation.fr(obsInd(i)) obsNo 0 totalDummyNo 1 cur_observation.g_id(obsInd(i))]);                                 

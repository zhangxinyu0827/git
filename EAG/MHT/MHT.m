...
[observation] = detectionConfOpt(observation)
% % 0731zxy剪取置信度较低的节点
 observation.r=eps+(observation.r(:)-min(observation.r(:)))/(max(observation.r(:))-min(observation.r(:)));
...
for k = firstFrame:lastFrame
    ...
    %zxy 170522
    cur_observation.g_id=observation.g_id(idx);
%    cur_observation.flag=observation.flag(idx);
    if other_param.isAppModel
        cur_observation.app = observation.app(idx,:);
    end        
%     tmpp= length(cur_observation.x);
    if (strcmp(other_param.seq,'MOT_Challenge_train') || strcmp(other_param.seq,'MOT_Challenge_test') ...
            || strcmp(other_param.seq,'MOT_Challenge_test_16') ||strcmp(other_param.seq,'MOT_Challenge_train_16'))
            
       cur_observation = NMS(cur_observation, other_param); 
    end
% if tmpp~= length(cur_observation.x)
%     1
% end

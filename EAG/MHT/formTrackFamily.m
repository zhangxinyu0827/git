...
%     observationNo = length( find(cur_observation.flag));
...
%             loglik = log(cur_observation.sc(i)/(1-cur_observation.sc(i)+eps));
            ...
            obsTreeSet(ct) = tree([cur_observation.x(i) cur_observation.y(i) cur_observation.w(i) cur_observation.h(i) cur_observation.fr(i) 1 0 0 1 cur_observation.g_id(i)]);       % last four elements : (1) the number of observation nodes (2) the number of dummy nodes (3) the number of total dummy nodes (4) a dummy node indicator        
...
%                 loglik=loglik+0.5;
           ...
           obsTreeSetPrev(i) = obsTreeSetPrev(i).addnode(treeInd(j),[previousObservation(1) previousObservation(2) previousObservation(3) previousObservation(4) cur_time obsNo dummyNo totalDummyNo NaN previousObservation(10)]); 
           ...
           if other_param.isAppModel
             ...
             tsp_sel=(full(cur_mask( previousObservation(10),cur_observation.g_id)))==1;
              % appearance-based pruning 
              obsSel = vbar >= other_param.appTH;   
               obsSel=(tsp_sel')|obsSel;
           else
              vbar = [];
              obsSel = [];
           end
...
%            end
%zxy0529
%            elseif (totalDummyNo-dummyNo)/(obsNo+totalDummyNo-dummyNo) >= other_param.dummyRatioTH
%                ct1 = ct1+1;
           elseif  confsc/obsNo <= other_param.confscTH || (totalDummyNo-dummyNo)/(obsNo+totalDummyNo-dummyNo) >= other_param.dummyRatioTH
        
%                tmp_cur_obs_tr=obsTreeSetPrev(i).Node;
%                tmp_obs_ind=[];
%                cur_node_for_tsp_ind=treeInd(j);
%                while cur_node_for_tsp_ind~=0
%                    tmp_obs_ind=[tmp_obs_ind  (tmp_cur_obs_tr{cur_node_for_tsp_ind,1}(:,10)')];
%                    cur_node_for_tsp_ind=idTreeSetPrev(i).getparent(cur_node_for_tsp_ind);
%                end
%                tmp_obs_ind=fliplr(tmp_obs_ind);
%                tsp_link_num=length(find(diag(cur_mask(tmp_obs_ind(1:end-1),tmp_obs_ind(2:end-1)))));
%                if tsp_link_num/obsNo>= other_param.dummyRatioTH
%                    ct2 = ct2+1; % count good tracks that have been confirmed
%                else
                   ct1 = ct1+1;
%                end
        ...
        % branch pruning
            ...
            activeTreeSetPrev(i) = activateTrackBranch(idTreeSetPrev(i),scoreTreeSetPrev(i),obsTreeSetPrev(i),activeTreeSetPrev(i),other_param,cur_time);            
        end
    end       

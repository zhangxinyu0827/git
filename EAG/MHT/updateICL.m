    ...
    for i = 1:familyNo
        ...
        for j = 1:length(treeInd) 
            ...
            if ~isnan(sel_observation1(9))
                conflictInd = getConfictInd([sel_observation1(1) sel_observation1(2) sel_observation1(3) sel_observation1(4)], cur_observation);
...
%                 iclSel = cell2mat(obsMembership(ind1 & ind2)); % [familyID branchIndex trackID]
                if numel(find(conflictInd == true)) == 0
                	error('Coffee: It would not happen, at least I think!');
                elseif numel(find(conflictInd == true)) > 1
                	display('Coffee: It works!');
                end

                iclSel = cell2mat(obsMembership(conflictInd)); % [familyID branchIndex trackID]
                if ~isempty(iclSel)                        

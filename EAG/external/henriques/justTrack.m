...
[rs, cs] = ndgrid((1:sz(1)) - floor(sz(1)/2), (1:sz(2)) - floor(sz(2)/2));%网格剖分，类似于meshgrid
y = exp(-0.5 / output_sigma^2 * (rs.^2 + cs.^2));
yf = fft2(y);%2维傅立叶变换

%store pre-computed cosine window
%对傅立叶变换加窗，截取傅立叶后减小频谱中由于
% 泄漏带来的测量不准确性
...
for frame = 1:numel(img_files),
    ...
    %extract and pre-process subwindow提取预处理过的子窗口
...
%     %% zxy1222,用于终止非正常迭代
%    im_del= [video_path img_files{frame}];
%    im_VB_del_file= [im_del(1:20) 'VB' im_del(22:end-3) 'bmp' ];
%    im_del = imread(im_VB_del_file);  
% %   if  im_VB_del_file(1:20)~='data/PETS09-S1L2/img'
%    try
%   del_box= im_del(rect_position(2):rect_position(2)+rect_position(4),...
%             rect_position(1):rect_position(1)+rect_position(3));
%    catch
%        break;
%    end
%    fore_ind=find(del_box(:));
%    if length(fore_ind)<0.1*length(del_box(:))
%        break;
%    end
%   end   

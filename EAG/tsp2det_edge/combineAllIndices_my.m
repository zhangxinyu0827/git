function [ISall,IMIND]=combineAllIndices_my(TSP,Iunsp,sceneInfo)
...
for t=1:F
    ...
    % flow %%%    
    if t==1
        tmp_num=sprintf('%06d_flow.mat',sceneInfo.frameNums(t+1));
       tmp_flow_f= [sceneInfo.imgFolder '/TSP_flows/' tmp_num];
        flow2=load(tmp_flow_f);
        
        meanflowX=flow2.flow.fvx;
        meanflowY=flow2.flow.fvy;
        
    elseif t==F
       tmp_num=sprintf('%06d_flow.mat',sceneInfo.frameNums(t));
       tmp_flow_f= [sceneInfo.imgFolder '/TSP_flows/' tmp_num];
        flow1=load(tmp_flow_f);

...
for t=1:F
    ...
    ndet=length(detections(t).sc);
%     im=getFrame(sceneInfo,t);
   im_tt=sceneInfo.frameNums(1)-1+t;
   im_ff=sprintf('%06d.jpg',im_tt);
   im_f=[sceneInfo.imgFolder im_ff];
   im=imread(im_f);
% im=iminfo(t).img;
    npix=size(im,1)*size(im,2);
    segs=unique(thisF(:))';
...
%     imwrite(Itmpc,sprintf('tmp/hyp/s%02d-f%04d-lhco.jpg',scenario,sceneInfo.frameNums(t)));
    imwrite(Itmp,sprintf('tmp/hyp/s%02d-f%04d-lh.jpg',scenario,frames(1)-1+t));
